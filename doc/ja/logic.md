...
- エラー状態のエージェントが打ち切りの人数に達した場合
  打ち切りの人数は `int(agent_count × server.max_continue_error_ratio)` で、切り捨てで0になる場合は1人に丸めます。
...
エージェントの `remain_length` が0以下の場合は、スキップします。 (`remain_length` が設定されていない場合は除きます。)\
エージェントに `WHISPER` リクエストを送信します。\
エージェントからのレスポンスを受信します。\
エラーが発生した場合は、スキップ発言に置換します。 (残りスキップ回数の消費は行いません。)\
残りスキップ回数は `max_skip` で初期化され、スキップ発言のたびに1減少します。\
残りスキップ回数が0の状態でスキップ発言が指定された場合は、オーバー発言に置換します。\
発言がオーバーもしくはスキップではない場合は、残りスキップ回数を `max_skip` にリセットします。\
