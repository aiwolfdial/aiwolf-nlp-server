...
## team_health (チーム健全性の設定)

> [!NOTE]
> チームごとの失敗率を直近の試合から集計し、マッチングの重みと隔離に反映するための機能です。\
> 集計結果は [GET /api/v1/teams](/doc/ja/api.md#get-apiv1teams) で確認できます。\
> `matching.is_optimize` が `false` の場合、集計と API への公開は行われますが、重みによる並べ替えと隔離は行われません。

失敗率は直近 `window` 試合における以下の3つの指標の加重平均で、0から1の値をとります。

- 応答エラー率: リクエストに対してタイムアウトやエラーが返った割合
- 脱落率: エージェントが以降のリクエストを一切受け付けられなくなった試合の割合
- 異常終了率: `server.max_continue_error_ratio` を超えて打ち切られた試合の割合

マッチの優先度は `scheduled_matches` の `weight` に、参加する各チームの重み (`1 - 失敗率`) を掛けた実効重みで決まります。\
マッチ単位の失敗とチーム単位の失敗を同じ重みの上で扱うため、どちらの理由でも優先度は同じ仕組みで下がります。

- `enable`: チーム健全性の集計を有効にするかどうか
- `window`: 失敗率の評価対象とする直近の試合数
- `min_games`: 評価を開始するのに必要な最小試合数
  これを下回るチームは減点も隔離もされません。1試合の事故で除外しないための猶予です。
- `scores`: 失敗率を求めるときの各指標の重み。比だけが意味を持ちます。
  - `request_error`: 応答エラー率の寄与
  - `fatal`: 脱落率の寄与
  - `abort`: 異常終了率の寄与
- `weight_floor`: チームの重みの下限
  0にすると事実上の永久除外になるため、通常は0より大きい値を指定します。
- `quarantine_rate`: 隔離を開始する失敗率の閾値
- `quarantine_duration`: 隔離を解除するまでの時間
  隔離中のチームを含むマッチはマッチングの候補から外れ、期限を過ぎると自動で復帰します。\
  ただし候補が1件も残らない場合に限り、ゲームが成立しなくなるのを避けるため隔離は無視されます。

> [!IMPORTANT]
> 隔離が実効を持つのは `matching.team_count` が `game.agent_count` より多い場合だけです。\
> 両者が等しいと生成されるマッチが必ず全チームを使うため、どのチームを隔離しても候補が全滅し、常に隔離が無視されます。\
> この場合でも重みによる優先度の低下と、API・Slack への可視化は機能します。
- `abort_weight_factor`: 異常終了したマッチ自体の `weight` に掛ける係数
  `0.0` の場合、異常終了したマッチは一度で最下位まで下がります。`0.5` のように0より大きい値を指定すると、失敗のたびに段階的に下がります。

## slack_notifier (Slack通知の設定)

> [!NOTE]
> 運用中に人が気づく必要のあるイベントを Slack の Incoming Webhook へ送ります。\
> 送信は専用のゴルーチンが行うため、Slack の遅延や障害がゲームの進行を止めることはありません。

通知は Block Kit で組み立て、重要度を色帯で区別します（緑=進捗、黄=隔離と停滞、赤=異常終了、青=起動と停止）。\
進捗と失敗率はバーで表示します。桁が揃うようバーはインラインコードで囲んでいます。

```
:chart_with_upwards_trend: 進捗  15 / 28 試合
███████████░░░░░░░░░  53.6%
残り 13 試合  ・  実行中 1 試合
```

- `enable`: Slack通知を有効にするかどうか
- `webhook_url`: Incoming Webhook の URL
  空の場合は環境変数 `SLACK_WEBHOOK_URL` を使用します。URLは秘匿情報のため、設定ファイルへ直接書かず環境変数を使うことを推奨します。
- `username`: 通知時の表示名
- `icon_emoji`: 通知時のアイコン
- `timeout`: Webhook への送信タイムアウト時間
- `min_interval`: 送信の最小間隔
  Webhook の流量制限に当たらないよう、この間隔を空けて送信します。
- `events`: 通知する種類の一覧。空の場合はすべて通知します。
  - `quarantine`: チームが隔離されたとき
  - `abort`: ゲームがエラー多発で打ち切られたとき
  - `stall`: マッチが一定時間成立していないとき
  - `milestone`: サーバの起動・シャットダウンと、一定試合数ごとの進捗
- `stall_threshold`: マッチが成立しない状態がこの時間続いたら通知します
  進行中のゲームが1件も無い場合にのみ判定します。0 の場合は停滞の監視を行いません。
- `milestone_every`: 何試合ごとに進捗を通知するか
  0 の場合は進捗の通知を行いません。
