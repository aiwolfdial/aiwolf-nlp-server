...
- `SLACK_WEBHOOK_URL`: 設定ファイルの `slack_notifier.enable` が `true` の場合に通知先となる Incoming Webhook の URL
  秘匿情報のため設定ファイルには置けません。ここでのみ指定します。
...
- `max_continue_error_ratio`: ゲームを継続するエラーエージェントの最大割合
  打ち切る人数は `int(agent_count × この値)` ですが、切り捨てで0になる場合は1人に丸めます。\
  0人だと脱落者がいなくてもゲームが即座に打ち切られてしまうためです（5人村の 0.2 未満、13人村の 0.077 未満が該当）。
...
- `infinite_loop`: 組み合わせマッチングがすべて終了した場合に全体のゲーム数分のゲームを追加するかどうか (`is_optimize` が `true` の場合に限る)
  基本的には `false` で問題ありません。
- `abort_weight_factor`: エラー多発で打ち切られたマッチ自身の重みに掛ける係数 (`is_optimize` が `true` の場合に限る)
  `0.0` の場合、異常終了したマッチは一度で最下位まで下がります。`0.5` のように0より大きい値を指定すると、失敗のたびに段階的に下がります。\
  マッチ単位の重みの設定なので、`team_health` の有効・無効とは独立して効きます。
...
- `quarantine_duration`: 隔離してから自動で解除されるまでの時間
  隔離中に更に失敗しても期限は延びません。延ばすと失敗が失敗を呼んで復帰できなくなるため、必ず一度は復帰させて直近の成績で再評価します。\
  復帰後もウィンドウ内に失敗が残っていれば、次の試合の結果で再び隔離されます。\
  隔離中のチームを含むマッチはマッチングの候補から外れます。\
  ただし候補が1件も残らない場合に限り、ゲームが成立しなくなるのを避けるため隔離は無視されます。

> [!IMPORTANT]
> チーム単位の評価が対戦順に影響するのは `matching.team_count` が `game.agent_count` より多い場合だけです。\
> 両者が等しいと生成されるマッチが必ず全チームを1席ずつ使うため、次の2つが同時に起きます。\
> ・どのチームを隔離しても候補が全滅し、常に隔離が無視されます。\
> ・チームの重みの積がどのマッチでも同じ値になり、並び順はマッチ自身の `weight` だけで決まります。\
> この構成では `matching.abort_weight_factor` によるマッチ単位の優先度低下と、API・Slack への可視化のみが機能します。
...
> Webhook の URL は秘匿情報のため設定ファイルには置かず、環境変数 `SLACK_WEBHOOK_URL` からのみ読みます。未設定の場合は警告を出して通知を無効にします。
...
- `stall_threshold`: 新しいマッチが成立しない状態がこの時間続いたら通知します
  待機部屋にチームがいる場合にのみ判定します。0 の場合は停滞の監視を行いません。\
  進行中のゲームがあっても、待たされているチームがいれば通知します。大会中はほぼ常に何かが走っているため、対戦中を除外するとほぼ発火しないためです。\
  通知には待機部屋にいる「接続中」のチームと、対戦表に載っているのに接続していない「接続待ち」のチームを分けて出します。\
  マッチが組めない原因は後者にあるためですが、名簿が対戦表由来なので `matching.is_optimize` が `false` の場合は接続中しか表示できません。
